...
const MODULE_PATH:string = `${Config.getProdFolderName()}/assets/scripts/color-picker`;


const styleUrls: string[] = [`${MODULE_PATH}/color-picker.css`];
const templateUrl: string = `${MODULE_PATH}/color-picker.html`;
...
export class ColorPickerDirective implements OnInit {
    ...
    @Input('cpHeight') cpHeight: string = '250px';
    private dialog: any;
    ...
    changeInput(value: string) {
        this.dialog.setColorFromString(value['value']);
        this.colorPickerChange.emit(value);
    }
}
...
export class DialogComponent implements OnInit {
    ...
    setDialog(instance: any, elementRef: ElementRef, color: any, cpPosition: string, cpPositionOffset: string,
       ...
       this.savedColors = this.service.savedColors;
    }

    public setOutputColor(color,button){

       this.setColorFromString(color.value);
       this.clickedButtonId = button.id;
        //this.directiveInstance.changeInput(color);
      
    }

    public addToCustomColors(){

         if(this.savedColors){
            let item = this.savedColors.find(el => (el.value === '#ffffff') || (el.value === '#fff'));
              if(item){
                this.savedColors.find(el => (el.value === '#ffffff') || (el.value === '#fff')).value = this.outputColor;
              }else{
                this.savedColors[this.savedColors.length -1].value = this.outputColor;
              }
        }
    ...
    deleteCustomColor(){
        ...
        if(this.clickedButtonId && this.savedColors){
            this.savedColors.find(el => el.id === this.clickedButtonId).value = "#fff";
        }
        else if(this.savedColors){
            var filtered = this.savedColors.filter(el => (el.value !== '#ffffff') && (el.value !== '#fff'));

             if(!_.isEmpty(filtered)){
               this.savedColors.find(el => el.id === filtered.slice(-1)[0].id).value = "#fff";
            }       
        }
    ...
    ngOnInit() {
       ...
       // this.setColorFromString(this.outputColor);
    ...
    openColorPicker() {
        if (!this.show) {
            ...
            this.hsva = this.service.stringToHsva(this.directiveInstance.colorPicker);
            this.update();
        }
    }
